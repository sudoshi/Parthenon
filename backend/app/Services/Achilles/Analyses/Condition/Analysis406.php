...
/**
 * Analysis 406: Distribution of age at first condition occurrence, by gender.
 */
class Analysis406 implements AchillesAnalysisInterface
{
    ...
    {
        return 'Distribution of age at first condition occurrence by gender';
    }
    ...
    {
        return <<<'SQL'
            DELETE FROM {@resultsSchema}.achilles_results_dist WHERE analysis_id = 406;
            ...
            FROM (
                SELECT co.person_id,
                    FLOOR(EXTRACT(YEAR FROM AGE(MIN(co.condition_start_date),
                ...
                GROUP BY co.person_id, p.year_of_birth, p.month_of_birth, p.day_of_birth
            ) t
