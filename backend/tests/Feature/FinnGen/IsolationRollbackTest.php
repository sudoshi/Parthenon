...
it('migrate→rollback→migrate preserves schema and round-trips row counts', function (): void {
    ...
    try {
        ...
        // Mandatory suite cleanup: this test deliberately runs migration DDL
        // outside transactions, so reset the database before any later tests.
        foreach (['pgsql_testing', 'pgsql', 'finngen', 'finngen_ro', 'omop', 'vocab', 'results', 'gis'] as $connection) {
            DB::purge($connection);
        }
