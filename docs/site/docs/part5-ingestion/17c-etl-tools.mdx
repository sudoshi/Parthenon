...
The Source Profiler tab provides a streamlined version of the [full Source Profiler](./17a-source-profiler) for quick database scanning. It connects to the WhiteRabbit service to scan your data source and produce a table-by-table quality report.
...
The Source Profiler tab here provides the essential scanning workflow. For advanced features like the completeness heatmap, data quality scorecard with letter grades, table size distribution chart, scan history, and sorting/filtering controls, use the dedicated [Source Profiler page](./17a-source-profiler).
:::
...
The FHIR Ingestion tab provides an embedded version of the [FHIR Ingestion tool](./17b-fhir-ingestion) for converting FHIR R4 resources into OMOP CDM records. It supports both JSON Bundle paste and NDJSON file upload, with resource preview, mapping coverage metrics, and error logging.

For complete documentation of the FHIR ingestion workflow, resource type mappings, and API reference, see the [dedicated FHIR Ingestion page](./17b-fhir-ingestion).
...
If a service is unavailable, its corresponding tab will display a warning and operations will fail. Check the [System Health Dashboard](/administration/system-health) for service status details.
...
- [Source Profiler (full)](./17a-source-profiler) — Advanced profiling with heatmap, scorecard, and history
- [FHIR Ingestion (full)](./17b-fhir-ingestion) — Complete FHIR workflow with API reference
- [Schema Mapping](./16-schema-mapping) — Mapping source schemas to OMOP CDM
- [Concept Mapping](./17-concept-mapping) — Mapping source codes to OMOP concepts
