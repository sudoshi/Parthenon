---
slug: r-runtime-hardening
title: "Hardening the R Runtime: From Single-Threaded Fragility to Production-Grade Infrastructure"
authors: [mudoshi, claude]
tags: [development, r-runtime, infrastructure, plumber2, mirai, async, hades, ohdsi, docker, devops]
date: 2026-03-19
---
...
I had been patching around the R runtime's limitations for weeks: extending health check intervals to avoid false-positive container restarts, adding retry logic in the Laravel `RService`, telling users to "wait for the current analysis to finish." But the core problem was architectural: Plumber v1 is single-threaded by design, and no amount of application-level workarounds fixes that.

The `parthenon-r` container runs all HADES analyses — CohortMethod, PatientLevelPrediction, SCCS, CohortDiagnostics, Characterization, Strategus, and more. When one user kicks off a 20-minute propensity score matching job, every other user is locked out of the entire R API. Health checks fail. Docker marks the container unhealthy. The container restarts. The running analysis dies. The user sees "failed."

## Before: What Was Broken

The architecture before this hardening was simple to a fault:
...
### Specific Failure Modes Observed in Production
...
4. **JDBC driver present:** Checks that `/opt/jdbc/postgresql-42.7.3.jar` exists. This file lives outside `/app` because docker-compose mounts `./r-runtime:/app` at runtime, which would clobber anything at `/app/jdbc`. (This was a real bug I hit during early development — the JDBC driver "disappeared" because the volume mount overwrote it.)
...
I also added a `.Last` shutdown handler in `plumber_api.R`:
...
Then replaced every bare `DatabaseConnector::disconnect(connection)` call across 6 API files with `safe_disconnect(connection)`. The affected files:
...
This was the most ambitious change and the one I debated the longest.

### The Decision: valve vs. plumber2
...
The Dockerfile changes were substantial.
...
The entrypoint changes from bare `Rscript` to s6-overlay's `/init`, which provides proper PID 1 behavior (signal forwarding, zombie reaping) and automatic service restart on crash.
...
### mirai Daemon Launcher

The `valve_launcher.R` file starts 3 mirai worker daemons before sourcing the plumber2 API:
...
Each daemon is a separate R process that inherits the parent's library paths and the pre-warmed namespace cache from the Docker build. Each daemon loads HADES packages + JVM on first use, consuming ~3GB. With 3 daemons that's ~9GB, well within the 32GB container limit.
...
In plumber2, the request body is passed directly as the `body` parameter. The response object is `response`, not `res`.

**2. Route annotations — full path required:**

I initially tried using plumber2's `@root` annotation to set path prefixes per file, expecting it to work like plumber v1's mount points. It didn't — routes ended up at `/run` instead of `/analysis/estimation/run`. The fix was to inline the full path in every endpoint annotation:
...
This phase had five significant build issues worth documenting:
...
| `r-runtime/R/connection.R` | JDBC URL timeouts, `safe_disconnect()` helper |
| `r-runtime/R/progress.R` | `safe_execute()` parameter renamed `res` -> `response` |
| `r-runtime/api/estimation.R` | plumber2 syntax, full path annotations, safe_disconnect |
...
1. **callr workers don't share mirai daemons.** The callr-based job registry (`async_jobs.R`) spawns independent R processes, not mirai daemon tasks. This means a callr job and a mirai-dispatched request use separate processes and separate memory. In practice this is fine — callr is for the Laravel async flow, mirai is for direct HTTP concurrency — but the container could theoretically run 3 mirai daemons + N callr processes simultaneously. Memory is the constraint (~3GB per process).
