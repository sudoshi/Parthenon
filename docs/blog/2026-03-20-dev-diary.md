...
title: "Workbench Launcher, Aqueduct Removal, and the Single-Database Migration Plan"
authors: [mudoshi, claude]
tags: [development, ohdsi, analytics, frontend, backend, infrastructure, database]
date: 2026-03-20
---

A big architectural day on Parthenon: we shipped the new **Workbench launcher experience**, ripped out the long-deprecated Aqueduct subsystem entirely, and drafted the formal plan to collapse our multi-database mess into a single, schema-isolated `parthenon` database. Sixty-eight commits, one repo, and a noticeably cleaner codebase on the other side.

<!-- truncate -->

## Workbench: From Dropdown to Launcher

The old FinnGen entry point was awkward — a toolset dropdown buried inside the FinnGen UI that never made much sense to new users and had quietly accumulated dead Aqueduct promo copy. Today we replaced it with a proper **Workbench launcher**.
...
**Inside FinnGen**, the toolset dropdown has been replaced with a simple back-to-Workbench link. No more Aqueduct promo copy. The UI is significantly less cluttered.

---

## Aqueduct Is Gone

Aqueduct has been deprecated in practice for a while, but the code was still sitting in the codebase accreting dust. Today we did the full removal — both sides of the stack:

- **Backend:** controller, service, models, routes, migrations, and tests all deleted.
- **Frontend:** Aqueduct UI components removed.

Native ingestion tooling has fully replaced whatever Aqueduct was doing. Keeping dead code around is a liability — it confuses new developers, inflates test surface area, and occasionally causes subtle conflicts. This is a clean break.
...
Today felt like a good cleanup-and-foundation day. Aqueduct is finally buried, the Workbench has real bones, and we have a credible plan for a database architecture that won't bite us anymore.