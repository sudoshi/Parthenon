...
- The full frontend scanner remains intentionally broader than the release-priority scope: `npm run i18n:scan` now reports 3,878 candidates across 1,103 files; excluding FinnGen paths leaves 3,506 candidates. The largest non-FinnGen clusters are ETL/CDM schema labels, Standard PRO instruments and builder UI, imaging, Strategus, investigation, profiles/patient similarity, publish/care gaps/risk-score surfaces, genomics, HEOR, Morpheus constants, and technical-tooling surfaces such as Poseidon/code tools.
...
- [x] Triage the full frontend scanner backlog into release-blocking user surfaces, generated/static data surfaces, intentional non-translatable strings, and later extraction waves. Current scan after the full analytics wave: 3,878 total candidates, 372 FinnGen candidates excluded from this i18n track, and 3,506 non-FinnGen candidates. Detailed triage and extraction order are captured in `docs/superpowers/specs/2026-04-20-i18n-full-scanner-backlog-triage.md`.
...
- [x] Complete the analysis design and results extraction wave from the full scanner. `src/features/analyses`, `src/features/estimation`, `src/features/prediction`, `src/features/pathways`, `src/features/sccs`, `src/features/self-controlled-cohort`, and `src/features/evidence-synthesis` now report 0 candidates in the focused analytics wave command across 75 files; the full scanner is down to 3,878 total candidates and 3,506 non-FinnGen candidates.
