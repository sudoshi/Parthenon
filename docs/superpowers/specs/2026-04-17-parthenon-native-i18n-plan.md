...
- Continued the post-extraction drafting/backfill phase with the imaging/genomics/radiogenomics tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/imagingGenomicsResources.ts` now carries native overrides across the imaging hub, study browser/detail/viewer workflow, DICOM import surfaces, criteria builder, measurement extraction/templates, longitudinal timeline and response-assessment flows, plus the molecular genomics upload/ClinVar/criteria/analysis/tumor-board chrome and the radiogenomics precision-medicine panel. Current combined distinct counts are `fr-FR` 478/569, `de-DE` 501/569, and `pt-BR` 503/569 (with imaging at `284/324`, `295/324`, `298/324`; genomics at `155/199`, `165/199`, `165/199`; and radiogenomics at `39/46`, `41/46`, `40/46`). The remaining identical strings are now concentrated in the expected technical/proper/same-spelling tail such as `DICOM`, `OHIF`, `OMOP`, `PACS`, `UID`, `NLP`, `RECIST`, `RANO`, `ClinVar`, `VCF`, `MAF`, `FHIR`, `HGVS`, `TMB`, `MSI`, `VUS`, `AF`, short placeholders like `CT, MR...` / `p.Leu858Arg`, and a smaller set of same-spelling UI cognates like `Status`, `Series`, `Gene`, `Date`, `Description`, and `Details`. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now improves overall distinct coverage to `fr-FR` 74.86%, `de-DE` 76.23%, and `pt-BR` 77.30%.
- Continued the post-extraction drafting/backfill phase with the Morpheus tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/morpheusResources.ts` now carries native overrides across the inpatient population dashboard, patient-journey tabs, shared event counts, concept drawer labels, diagnoses/distribution/trend chrome, dataset truncation copy, microbiology/culture/antibiogram sections, location track, medication timeline, labs panels/severity labels, and vital-sign timeline surfaces. Current Morpheus distinct counts are `fr-FR` 164/215, `de-DE` 184/215, and `pt-BR` 187/215; the remaining identical strings are now concentrated in the expected technical/proper/same-spelling tail such as the `Morpheus` brand, abbreviations like `ICU`, `LOS`, `MIC`, `ICD`, `MAP`, `SpO2`, `GCS`, short suffix tokens like `pts`, `adm`, `d`, `h`, specimen/location acronyms like `ED` and `PACU`, and a smaller set of genuine same-spelling clinical/UI labels like `patients`, `Description`, `Date`, `Dose`, and `Sequence`. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now improves overall distinct coverage to `fr-FR` 76.58%, `de-DE` 78.16%, and `pt-BR` 79.27%.
- Continued the post-extraction drafting/backfill phase with the GIS/tools tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/gisToolsResources.ts` now carries native overrides across the Poseidon orchestration dashboard, Jupyter workspace shell, Code Explorer counts/hierarchy/reporting flows, GIS explorer layer/region/detail chrome, and Query Assistant library/natural-language/results/schema/sql-runner surfaces. Current combined distinct counts are `fr-FR` 371/414, `de-DE` 383/414, and `pt-BR` 385/414 (with Poseidon at `66/76`, `72/76`, `70/76`; Jupyter at `32/37`, `32/37`, `32/37`; Code Explorer at `61/69`, `64/69`, `66/69`; GIS at `90/108`, `96/108`, `95/108`; and Query Assistant at `122/124`, `119/124`, `122/124`). The remaining identical strings are now concentrated in the expected technical/proper/same-spelling tail such as `Poseidon`, `Jupyter`, `SQL`, `dbt`, `Dagster`, `OMOP`, `CDM`, `ROMOPAPI`, `PM2.5`, `SVI`, `RUCC`, `CFR`, `null`, schema/runtime labels like `runtime`, `staging`, `cdm`, and a small set of genuine same-spelling UI terms like `Concept`, `Source`, `Status`, `Top`, `Region`, and `prevalence`. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now improves overall distinct coverage to `fr-FR` 80.47%, `de-DE` 82.18%, and `pt-BR` 83.31%.
- Continued the post-extraction drafting/backfill phase with the concept-set/shared-primitives tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/conceptSetResources.ts` now carries native overrides across the concept-set landing page, list/grid states, builder/editor controls, concept detail tabs, Phoebe recommendation chrome, bundle creation flow, import workflow, significance verdicts, and shared workbench run-status copy. Current concept-set distinct counts are `fr-FR` 119/133, `de-DE` 125/133, and `pt-BR` 127/133; the remaining identical strings are now concentrated in the expected same-spelling or intentionally preserved tail such as `Phoebe`, `Atlas`, `JSON`, `obs`, and a small set of short UI terms that remain identical or near-identical in context like `Standard`, `Info`, `Total`, `Tags`, `concept`, and `Status:`. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now improves overall distinct coverage to `fr-FR` 81.72%, `de-DE` 83.49%, and `pt-BR` 84.64%.
- Continued the post-extraction drafting/backfill phase with the data-sources/ingestion/FHIR tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/dataSourceIngestionResources.ts` now carries native overrides across the clinical-data-models registry/import wizard, access-control and connection-detail flow, ingestion project upload/staging/mapping/validation chrome, and the Vulcan/FHIR workspace plus FHIR-to-OMOP ingestion surfaces. Current data-source/ingestion distinct counts are `fr-FR` 427/457, `de-DE` 427/457, and `pt-BR` 425/457; the remaining identical strings are now concentrated in the expected technical/proper/same-spelling tail such as `WebAPI`, `CDM`, `Temp`, `Port`, `Schema`, database engine names like `PostgreSQL`, `SQL Server`, `BigQuery`, `DuckDB`, and workbench/product labels such as `Aqueduct`, `Poseidon`, `Vulcan`, `FHIR R4`, `Admin -> FHIR Connections`, plus compact tokens like `S`, `err`, and `IDs`. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now improves overall distinct coverage to `fr-FR` 84.83%, `de-DE` 86.56%, and `pt-BR` 87.68%.
- Continued the post-extraction drafting/backfill phase with a heavier second-pass analytics tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/analysisResources.ts` now adds another native overlay across calibration/discrimination copy, cohort-incidence direct-run messaging, hazard/rate ratio descriptions, risk-window and propensity-score diagnostics, external validation explanations, and the remaining high-frequency analysis labels that were still falling back to English. Current analysis distinct counts are `fr-FR` 375/552, `de-DE` 387/552, and `pt-BR` 389/552; the remaining identical strings are now concentrated in the expected technical/proper/same-spelling tail such as `AUC`, `AUPRC`, `IRR`, `Kaplan-Meier`, `LightGBM`, `OHDSI`, `SCCS`, short axis/placeholder tokens, and a smaller set of still-pending advanced statistical labels. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now improves overall distinct coverage to `fr-FR` 86.84%, `de-DE` 88.61%, and `pt-BR` 89.72%.
- Continued the post-extraction drafting/backfill phase with a broader cohort-authoring/triage tranche for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/cohortDefinitionResources.ts` now adds another native overlay across review-session setup, read-only sharing copy, imaging/molecular/risk-score filter prompts, follow-up strategy language, overlap/attrition diagnostics, data-source guardrails, and phenotype-validation / promotion-governance surfaces. Current cohort-definition distinct counts are `fr-FR` 385/557, `de-DE` 388/557, and `pt-BR` 391/557; the remaining identical strings are now concentrated in the expected technical/proper/same-spelling tail such as `Abby AI`, `Circe Compiler`, `PheValuator`, structured JSON examples, `Jaccard`, and a smaller set of long-tail wizard and review-workflow labels still awaiting a later cleanup pass. `npm run i18n:report` remains at 100% key presence / 0 missing keys, with the overall distinct coverage staying at `fr-FR` 86.84%, `de-DE` 88.61%, and `pt-BR` 89.72% after the combined batch.
- Continued the post-extraction drafting/backfill phase with the small-workbench and legacy Abby tranches for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`). `frontend/src/i18n/smallWorkbenchResources.ts` now carries native overrides for Study Designer, phenotype-library search/import, the Community Workbench SDK demo, Workbench launcher states, toolset metadata, and the existing ETL launcher slice. Current small-workbench distinct counts are `fr-FR` 118/128, `de-DE` 121/128, and `pt-BR` 122/128. `frontend/src/i18n/abbyLegacyResources.ts` now carries native overrides across the Abby AI panel, example prompts, concept-set counts, refinement flow, action plan, and research-profile panel. Current Abby legacy distinct counts are `fr-FR` 37/42, `de-DE` 39/42, and `pt-BR` 41/42.
- Completed the first 90%+ milestone for the French/German/Brazilian Portuguese native drafting lane by adding a targeted French cleanup overlay across `frontend/src/i18n/analysisResources.ts` and `frontend/src/i18n/cohortDefinitionResources.ts`. The cleanup covers the remaining translatable analytics labels, calibration/risk-window copy, review workflow labels, cohort-wizard step text, phenotype-validation governance copy, and explanatory cohort-authoring prompts while preserving product names, acronyms, placeholders, and standards identifiers. Current focused counts are `analysisResources` at `fr-FR` 489/552, `de-DE` 387/552, `pt-BR` 389/552 and `cohortDefinitionResources` at `fr-FR` 500/557, `de-DE` 388/557, `pt-BR` 391/557. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now reports overall distinct coverage at `fr-FR` 90.78%, `de-DE` 90.20%, and `pt-BR` 91.33%.
- Continued the 90%+ cleanup lane by extending the same high-value analytics and cohort-authoring overlays to German (`de-DE`) and Brazilian Portuguese (`pt-BR`) while preserving the completed French (`fr-FR`) cleanup. `frontend/src/i18n/analysisResources.ts` now covers covariance/SMD reference guidance, dechallenge/rechallenge, assumption failures, model/matching labels, power/statistical labels, and propensity-score/risk-window copy for all three locales; `frontend/src/i18n/cohortDefinitionResources.ts` now covers descendants, review workflow, risk-score filtering, wizard step text, specialized criteria, observation/medication/follow-up prompts, and empty-state/cohort-generation copy for all three locales. Current focused counts are `analysisResources` at `fr-FR` 489/552, `de-DE` 496/552, `pt-BR` 501/552 and `cohortDefinitionResources` at `fr-FR` 500/557, `de-DE` 500/557, `pt-BR` 505/557. `npm run i18n:report` remains at 100% key presence / 0 missing keys and now reports overall distinct coverage at `fr-FR` 90.78%, `de-DE` 92.52%, and `pt-BR` 93.70%.
- Completed the residue pass for French (`fr-FR`), German (`de-DE`), and Brazilian Portuguese (`pt-BR`) by finishing the remaining true-English app copy across `frontend/src/i18n/resources.ts`, `frontend/src/i18n/appResources.ts`, `frontend/src/i18n/analysisResources.ts`, `frontend/src/i18n/cohortDefinitionResources.ts`, `frontend/src/i18n/dashboardResources.ts`, `frontend/src/i18n/gisToolsResources.ts`, `frontend/src/i18n/profileSimilarityResources.ts`, and `frontend/src/i18n/publishCareGapRiskResources.ts`. This pass also added `frontend/src/i18n/completenessPolicy.ts` plus an updated `frontend/scripts/i18n-completeness.mjs` so canonical same-value strings such as brands, standards, acronyms, placeholders, paths, and same-spelling native terms are credited instead of being miscounted as untranslated residue. `npm run i18n:report` now reports `fr-FR`, `de-DE`, and `pt-BR` at `100% distinct`, while preserving `100%` key presence and `0` missing keys for every locale.
- Started the next public-locale drafting lane for Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) with the smaller workbench/concept tranche plus a targeted extracted-app cleanup. `frontend/src/i18n/conceptSetResources.ts` now carries native overrides for concept-set list/builder/detail/import/bundle flow and shared significance/workbench copy (`es-ES` `133/133`, `ko-KR` `132/133`, `hi-IN` `132/133` distinct). `frontend/src/i18n/smallWorkbenchResources.ts` now carries native overrides for Study Designer, phenotype-library search/detail/import, Community Workbench SDK demo copy, Workbench launcher states, toolset metadata, and the Aqueduct ETL launcher slice (`es-ES` `127/128`, `ko-KR` `128/128`, `hi-IN` `126/128` distinct). `frontend/src/i18n/abbyLegacyResources.ts` now carries native overrides across the Abby AI panel, example prompts, refinement flow, action plan, and research-profile panel (`42/42` distinct for all three locales). `frontend/src/i18n/appResources.ts` and `frontend/src/i18n/completenessPolicy.ts` were also extended to clear or correctly credit the small extracted-app residue that remained in studies/administration/data-explorer surfaces, bringing `appResources` to `es-ES` `2860/2862`, `ko-KR` `2862/2862`, and `hi-IN` `2862/2862` distinct. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `53.52%`, `ko-KR` `53.42%`, and `hi-IN` `53.40%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete Morpheus tranche in `frontend/src/i18n/morpheusResources.ts`. The file now carries native overrides across the Morpheus dashboard, patient-journey tabs and list chrome, event counts, concept drawer, culture/antibiogram surfaces, location track, medication timeline, labs, and vital-sign timelines for all three locales, taking Morpheus to `215/215` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `frontend/src/i18n/completenessPolicy.ts` was also extended to credit canonical same-value abbreviations like `ICU:` and `LOS:` rather than misclassifying them as untranslated residue. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `55.27%`, `ko-KR` `55.18%`, and `hi-IN` `55.16%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete HEOR tranche in `frontend/src/i18n/heorResources.ts`. The file now carries native overrides across the HEOR hub, economic-analysis and value-contract workflows, cost/QALY labels, scenario and claims copy, budget-impact and cost-effectiveness charts, tornado/sensitivity text, and the claims explorer chrome for all three locales, taking HEOR to `183/183` distinct for `es-ES`, `ko-KR`, and `hi-IN` (with only 4 protected same-value abbreviations per locale). `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `56.78%`, `ko-KR` `56.69%`, and `hi-IN` `56.67%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete ETL/Aqueduct tranche in `frontend/src/i18n/etlAqueductResources.ts`. The file now carries native overrides across the source-profiler workflow, scan-history and data-quality scorecard chrome, relationship and completeness labels, Aqueduct mapping/project states, AI suggestion copy, field-mapping controls, and export/documentation labels for all three locales, taking ETL/Aqueduct to `189/189` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `frontend/src/i18n/completenessPolicy.ts` was also extended to credit locale-native same-spelling Spanish `No` so the completeness report stops misclassifying that exact button label as untranslated residue. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `58.28%`, `ko-KR` `58.14%`, and `hi-IN` `58.12%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete Strategus tranche in `frontend/src/i18n/strategusResources.ts`. The file now carries native overrides across the study-package wizard, shared-cohort flow, validation/review and execution states, JSON preview/editor chrome, module-parameter sections and field labels, and the OHDSI module labels/descriptions for all three locales, taking Strategus to `167/167` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `59.91%`, `ko-KR` `59.78%`, and `hi-IN` `59.76%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete data-sources/ingestion/FHIR tranche in `frontend/src/i18n/dataSourceIngestionResources.ts`. The file now carries native overrides across the clinical-data-model registry/import/access-control wizard, database connection and daimon setup flow, ingestion project upload/staging/schema-mapping/review/validation chrome, and the Vulcan/FHIR workspace plus FHIR-to-OMOP ingestion surfaces for all three locales, taking `dataSourceIngestionResources` to `457/457` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `62.76%`, `ko-KR` `62.62%`, and `hi-IN` `62.59%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete publish/care-gaps/risk-scores tranche in `frontend/src/i18n/publishCareGapRiskResources.ts`. The file now carries native overrides across the risk-score hub/create/detail/results workflow, care-gap bundle design/compliance/population surfaces, and publish document selection/configuration/preview/export flow for all three locales, taking `publishCareGapRiskResources` to `572/572` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `frontend/src/i18n/completenessPolicy.ts` was also extended to credit locale-native same-spelling Spanish `Cardiovascular`, so the report no longer misclassifies that valid medical cognate as untranslated residue. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `67.86%`, `ko-KR` `67.70%`, and `hi-IN` `67.67%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with the patient-profiles half of `frontend/src/i18n/profileSimilarityResources.ts`. The file now carries native overrides across the patient profile page, cohort browser, person/MRN search, demographic header stats, concept detail drawer, labs, notes, visits, eras, and timeline chrome for all three locales, while the separate `patientSimilarity` workspace half remains the next pending tranche inside this file. The translated `profiles` subtree is now at `0` residue for `es-ES`, `ko-KR`, and `hi-IN`, and the file-level distinct counts improve to `es-ES` `313/640`, `ko-KR` `311/640`, and `hi-IN` `311/640`. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `69.48%`, `ko-KR` `69.33%`, and `hi-IN` `69.30%`.
- Completed the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) tranche for the remaining `patientSimilarity` workspace half of `frontend/src/i18n/profileSimilarityResources.ts`. The file now carries native overrides across the similarity workspace modes, pipeline steps, patient comparison narrative, selector bar, cohort expansion/export, diagnostics, divergence/charts, covariate balance, PSM, patient landscape, phenotype discovery, network fusion, and researcher-interpretation surfaces for all three locales, taking `profileSimilarityResources` to `640/640` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `72.91%`, `ko-KR` `72.78%`, and `hi-IN` `72.75%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with the full `imaging` subtree inside `frontend/src/i18n/imagingGenomicsResources.ts`. The file now carries native overrides across the imaging hub, study browser, DICOM import modal, measurement extraction/templates, OHIF viewer chrome, longitudinal patient timeline, and response-assessment workflow for all three locales, taking the `imaging` subtree itself to `324/324` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `frontend/src/i18n/completenessPolicy.ts` was also extended so canonical clinical criteria labels like `RECIST 1.1` and `Deauville/Lugano`, plus genuine same-spelling Spanish clinical terms, are credited correctly instead of being miscounted as untranslated residue. The combined `imagingGenomicsResources` file now improves to `es-ES` `369/569`, `ko-KR` `367/569`, and `hi-IN` `367/569` distinct before the later genomics/radiogenomics follow-on tranche. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `75.59%`, `ko-KR` `75.48%`, and `hi-IN` `75.45%`.
- Completed the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) follow-on tranche for the remaining `genomics` and `radiogenomics` sections inside `frontend/src/i18n/imagingGenomicsResources.ts`. The file now carries native overrides across variant upload/detail flow, ClinVar sync and browse surfaces, genomic cohort criteria, mutation-survival and treatment-matrix analysis, the molecular tumor-board workflow, and the radiogenomics precision-medicine panel for all three locales, taking `genomics` to `199/199`, `radiogenomics` to `46/46`, and the full `imagingGenomicsResources` file to `569/569` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `frontend/src/i18n/completenessPolicy.ts` was also extended so canonical genome-build and precision-genomics identifiers like `ClinVar:`, `GRCh38 / hg38`, `GRCh37 / hg19`, and `EGFR` are credited correctly instead of being miscounted as untranslated residue. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `77.69%`, `ko-KR` `77.60%`, and `hi-IN` `77.57%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a complete GIS/tools tranche in `frontend/src/i18n/gisToolsResources.ts`. The file now carries native overrides across the Poseidon orchestration dashboard, Jupyter workspace shell, Code Explorer counts/hierarchy/reporting flows, GIS explorer layer/region/detail chrome, and Query Assistant library/natural-language/results/schema/sql-runner surfaces for all three locales, taking `gisToolsResources` to `414/414` distinct for `es-ES`, `ko-KR`, and `hi-IN`. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `81.28%`, `ko-KR` `81.26%`, and `hi-IN` `80.77%`, with app-namespace distinct coverage at `es-ES` `79.31%`, `ko-KR` `79.30%`, and `hi-IN` `78.76%`.
- Continued the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane with a combined Standard PROs / investigation / analyses push across `frontend/src/i18n/standardProsResources.ts`, `frontend/src/i18n/investigationResources.ts`, and `frontend/src/i18n/analysisResources.ts`. This tranche adds native overrides across the Standard PROs library/builder/conduct/public-survey workflow, the evidence-investigation landing/phenotype/clinical/genomic/synthesis workflow, and the shared analytics launcher/result-state surfaces, then follows with a visible-copy cleanup pass so canonical identifiers such as `survey_conduct`, `survey:seed-library`, `Open Targets`, and Parthenon product names remain correct in translated UI copy. `npm run i18n:report` remains at `100%` key presence / `0` missing keys for every locale and now improves overall distinct coverage to `es-ES` `93.83%`, `ko-KR` `93.74%`, and `hi-IN` `93.68%`, clearing the next public-locale `90%+` milestone on `main`.
- After this milestone, the remaining residue for the `es-ES` / `ko-KR` / `hi-IN` lane is no longer broad app chrome; it is concentrated in `cohortDefinitions` (`533`, `535`, and `535` identical-to-source keys respectively), plus a small tail in `analyses` / `investigation` / `standardPros` for Spanish and Korean and a few smaller GIS/query-assistant/code-explorer leftovers for Hindi. The next highest-value tranche is therefore `frontend/src/i18n/cohortDefinitionResources.ts`, followed by those smaller cleanup tails.
- Completed the final residue pass for the same Spanish (`es-ES`), Korean (`ko-KR`), and Hindi (`hi-IN`) drafting lane by finishing the remaining true-English copy in `frontend/src/i18n/cohortDefinitionResources.ts`, `frontend/src/i18n/analysisResources.ts`, `frontend/src/i18n/standardProsResources.ts`, `frontend/src/i18n/investigationResources.ts`, `frontend/src/i18n/gisToolsResources.ts`, `frontend/src/i18n/smallWorkbenchResources.ts`, and `frontend/src/i18n/resources.ts`, while extending `frontend/src/i18n/completenessPolicy.ts` so canonical same-value strings like `SQL`, `SNOMED`, `PROMIS`, `GIS`, `Jupyter`, `Token`, `slug`, and locale-native same-spelling medical or technical terms are credited correctly instead of being miscounted as untranslated residue. This tranche also included a final visible-copy polish pass for Korean cohort-definition labels and small Spanish/Hindi shell strings so obvious machine-translation artifacts do not survive into the shipped UI. `npm run i18n:report` now reports `es-ES`, `ko-KR`, and `hi-IN` at `100%` distinct overall coverage with `100%` key presence and `0` missing keys, joining `fr-FR`, `de-DE`, and `pt-BR` as fully completed public app locales on `main`.
- Started the next public-locale drafting/backfill wave for Finnish (`fi-FI`), Japanese (`ja-JP`), and Simplified Chinese (`zh-Hans`) on `main` with a complete operational-pack tranche across `frontend/src/i18n/morpheusResources.ts`, `frontend/src/i18n/heorResources.ts`, `frontend/src/i18n/etlAqueductResources.ts`, `frontend/src/i18n/strategusResources.ts`, and `frontend/src/i18n/dataSourceIngestionResources.ts`. This pass translated the remaining true-English copy for Morpheus inpatient dashboards, HEOR analyses/contracts/claims, the BlackRabbit source-profiler and Aqueduct ETL designer, Strategus study-package authoring/execution, and the clinical-data-model / ingestion / FHIR onboarding surfaces for all three locales, then followed with a Finnish cleanup pass plus `frontend/src/i18n/completenessPolicy.ts` additions for legitimate same-value technical labels such as `ID`, `Daimons`, `Microsoft SQL Server`, `Amazon Web Services`, and `FHIR Bundle JSON`. These five files are now `100%` distinct for `fi-FI`, `ja-JP`, and `zh-Hans`, and `npm run i18n:report` still shows `100%` key presence / `0` missing keys for every locale while improving overall distinct coverage to `fi-FI` `59.41%`, `ja-JP` `59.80%`, and `zh-Hans` `59.82%`.
- Continued the same Finnish (`fi-FI`), Japanese (`ja-JP`), and Simplified Chinese (`zh-Hans`) drafting lane with a complete care-gaps / risk-scores / publish / patient-profiles / patient-similarity tranche in `frontend/src/i18n/publishCareGapRiskResources.ts` and `frontend/src/i18n/profileSimilarityResources.ts`. This pass translated the remaining true-English copy for risk-score hubs and detail views, care-gap bundle design/compliance/population workflows, publish document configuration/preview/export surfaces, patient profiles, cohort browser/search, concept detail, labs/notes/visits/eras/timeline views, and the full patient-similarity workspace including comparison, cohort expansion/export, diagnostics, landscape, phenotype discovery, network fusion, and researcher interpretation for all three locales, then followed with a targeted Finnish cleanup plus small Japanese/Chinese `headToHead` label fixes. Both files are now `100%` distinct for `fi-FI`, `ja-JP`, and `zh-Hans`, and `npm run i18n:report` still shows `100%` key presence / `0` missing keys for every locale while improving overall distinct coverage to `fi-FI` `69.57%`, `ja-JP` `69.96%`, and `zh-Hans` `69.98%`.
- Continued the same Finnish (`fi-FI`), Japanese (`ja-JP`), and Simplified Chinese (`zh-Hans`) drafting lane with a complete cohort-definition tranche in `frontend/src/i18n/cohortDefinitionResources.ts`. This pass translated the remaining true-English copy for cohort-definition authoring, wizard flows, inclusion/censoring logic, diagnostics, generation history, review and validation workflows, cohort overlap analysis, patient/member views, SQL preview, specialized genomics/imaging/risk-score criteria, and related clinical terminology for all three locales, then followed with a targeted Finnish residue cleanup so `Concept ID`, inclusion-rule attrition, min/max, label, and offset terms no longer fall back to English. `cohortDefinitionResources.ts` now reports `557/557` distinct for `fi-FI`, `ja-JP`, and `zh-Hans`, and `npm run i18n:report` still shows `100%` key presence / `0` missing keys for every locale while improving overall distinct coverage to `fi-FI` `75.18%`, `ja-JP` `75.57%`, and `zh-Hans` `75.59%`.
- Continued the same Finnish (`fi-FI`), Japanese (`ja-JP`), and Simplified Chinese (`zh-Hans`) drafting lane with the next analysis + imaging/genomics + concept-set tranche in `frontend/src/i18n/analysisResources.ts`, `frontend/src/i18n/imagingGenomicsResources.ts`, and `frontend/src/i18n/conceptSetResources.ts`. This pass translated the remaining true-English copy for the analysis workspace, imaging viewer/timeline/response flows, genomics and radiogenomics surfaces, and concept-set editing/Phoebe chrome for all three locales, then followed with a targeted manual cleanup pass for Finnish imaging/genomics residue plus Japanese and Simplified Chinese terminology fixes where machine output had left awkward or misleading strings. `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale, and overall distinct coverage now stands at `fi-FI` `86.15%`, `ja-JP` `86.54%`, and `zh-Hans` `86.56%`, safely clearing the 85% milestone for this trio.
- Continued the same Finnish (`fi-FI`), Japanese (`ja-JP`), and Simplified Chinese (`zh-Hans`) drafting lane with a combined Standard PROs / investigation / small-workbench / Abby legacy tranche in `frontend/src/i18n/standardProsResources.ts`, `frontend/src/i18n/investigationResources.ts`, `frontend/src/i18n/smallWorkbenchResources.ts`, and `frontend/src/i18n/abbyLegacyResources.ts`. This pass translated the remaining true-English copy for the Standard PROs library/builder/conduct/public-survey workflow, the evidence-investigation landing/phenotype/clinical/genomic/synthesis workflow, Study Designer and phenotype-library workbench chrome, and the Abby AI legacy panel/profile flow for all three locales, then followed with a targeted manual cleanup pass to remove placeholder leaks and awkward visible strings in Finnish, Japanese, and Simplified Chinese before shipping. The touched files now have `0` open same-as-English residue keys outside the expected protected canonical-value set, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving overall distinct coverage to `fi-FI` `95.50%`, `ja-JP` `95.89%`, and `zh-Hans` `95.91%`, pushing this trio cleanly into the mid-90s on `main`.
- Completed the final residue pass for Finnish (`fi-FI`), Japanese (`ja-JP`), and Simplified Chinese (`zh-Hans`) by finishing the remaining shared-shell and tooling copy in `frontend/src/i18n/resources.ts`, `frontend/src/i18n/appResources.ts`, `frontend/src/i18n/commonsResources.ts`, and `frontend/src/i18n/gisToolsResources.ts`. This pass closed the last untranslated navigation labels, settings/profile notification text, Abby memorial/about prose, Commons presence labels, GIS / Poseidon / Jupyter / Query Assistant chrome, and small administration/data-explorer shell tails for all three locales, then followed with a targeted manual cleanup pass for the remaining Finnish/Japanese/Chinese operator-facing strings so the last visible rough edges did not survive into the shipped UI. `npm run i18n:report` now reports `fi-FI`, `ja-JP`, and `zh-Hans` at `100% distinct` overall coverage with `100%` key presence and `0` missing keys, joining `es-ES`, `fr-FR`, `de-DE`, `pt-BR`, `ko-KR`, and `hi-IN` as fully completed public app locales on `main`.
- Started the Arabic (`ar`) RTL-canary native drafting lane on `main` with a structured dashboard / Abby legacy / Strategus tranche across `frontend/src/i18n/dashboardResources.ts`, `frontend/src/i18n/abbyLegacyResources.ts`, `frontend/src/i18n/strategusResources.ts`, and the Arabic dashboard wiring in `frontend/src/i18n/resources.ts`. This pass translated the remaining true-English copy for the dashboard metrics / CDM characterization / empty states, the Abby AI legacy cohort-builder and research-profile flow, and the full Strategus study-package authoring / validation / execution workspace for Arabic, while preserving runtime placeholders and canonical identifiers like `CDM`, `JSON`, `OHDSI`, `CohortGenerator`, and `SCCS` where appropriate. The touched files now report `dashboardResources` `77/77`, `abbyLegacyResources` `42/42`, and `strategusResources` `167/167` distinct for `ar`, and `npm run i18n:report` still shows `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `21.44%`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a complete ETL / Aqueduct residue pass in `frontend/src/i18n/etlAqueductResources.ts`. This pass translated the remaining true-English copy across the BlackRabbit source-profiler workflow, scan history / heatmap / scorecard chrome, and the Aqueduct mapping / suggestions / field-mapping workspace for Arabic while preserving canonical identifiers such as `BlackRabbit`, `CDM`, `OMOP`, `SQL`, `FK`, and export-format tokens where appropriate. `etlAqueductResources.ts` now reports `189/189` distinct for `ar`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `22.85%`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a complete Morpheus inpatient-workflow residue pass in `frontend/src/i18n/morpheusResources.ts`. This pass translated the remaining true-English copy across Morpheus navigation, patient-journey tabs and list labels, dashboard metric/chart chrome, concept-drawer labels, culture / antibiogram surfaces, location track, medication timeline, labs, and vitals while preserving canonical identifiers such as `Morpheus`, `ICU`, `ICD`, `OMOP`, `CSV`, `CLSI M39`, and dataset-driven values where appropriate. `morpheusResources.ts` now reports `215/215` distinct for `ar`, and `npm run i18n:report` still shows `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `24.59%`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a complete HEOR residue pass in `frontend/src/i18n/heorResources.ts`. This pass translated the remaining true-English copy across the HEOR hub, cost-effectiveness / budget-impact / ROI analysis workflow, scenario and parameter forms, value-contract authoring, claims explorer facets and financial stats, and the budget-impact / CE plane / scenario comparison / tornado chart chrome for Arabic while preserving runtime placeholders and canonical terms such as `HEOR`, `QALY`, `ICER`, `NMB`, `WTP`, and `Solr` where appropriate. `heorResources.ts` now reports `183/183` distinct for `ar`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `26.09%`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a complete concept-set residue pass in `frontend/src/i18n/conceptSetResources.ts`. This pass translated the remaining true-English copy across the concept-set library, builder and editor controls, hierarchy / relationship / source-code detail tabs, Phoebe recommendation chrome, care-bundle generation flow, concept-set import dialog, and shared significance / workbench labels for Arabic while preserving runtime placeholders and canonical terms such as `Phoebe`, `Atlas`, and JSON structure examples where appropriate. `conceptSetResources.ts` now reports `133/133` distinct for `ar`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `27.23%`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a structured workbench / Commons / ingestion / Standard PROs tranche across `frontend/src/i18n/smallWorkbenchResources.ts`, `frontend/src/i18n/commonsResources.ts`, `frontend/src/i18n/resources.ts`, `frontend/src/i18n/dataSourceIngestionResources.ts`, and `frontend/src/i18n/standardProsResources.ts`. This pass translated the remaining true-English copy for the Study Designer and phenotype-library shell, Commons channel/call/wiki/source-attribution chrome, Arabic Commons resource wiring, data-source registry / connection / daimon setup flow, ingestion project and FHIR workspace surfaces, and the Standard PROs library / analytics / about / builder / conduct / instrument detail / public survey workflow for Arabic while preserving runtime placeholders and canonical identifiers such as `OMOP`, `FHIR`, `REDCap`, `LOINC`, `SNOMED`, and `PROMIS` where appropriate. `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale and now improves Arabic overall distinct coverage to `40%`, clearing the requested next RTL-canary milestone on `main`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a GIS/tools tranche across `frontend/src/i18n/gisToolsResources.ts` and the canonical-label credit update in `frontend/src/i18n/completenessPolicy.ts`. This pass translated the remaining true-English copy for the Poseidon orchestration dashboard, Jupyter workbench shell, code-explorer counts / hierarchy / reports flow, GIS explorer layer and regional-detail surfaces, and the Query Assistant library / natural-language / schema-browser / SQL-runner workflow for Arabic while preserving reviewed canonical values such as `Poseidon`, `CDM`, `Parthenon Jupyter`, `JupyterLab 4.4`, `concept_id`, `CFR`, `PM2.5`, `SVI`, `SQL`, `null`, fixed filesystem paths, and runtime placeholder formats where appropriate. `frontend/src/i18n/gisToolsResources.ts` now carries `400/414` distinct Arabic values, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `43.67%`.
- Continued the Arabic (`ar`) RTL-canary lane on `main` with a focused investigation + risk/publish tranche across `frontend/src/i18n/investigationResources.ts` and `frontend/src/i18n/publishCareGapRiskResources.ts`. This pass translated the remaining true-English copy for the investigation shell, landing workflow, phenotype authoring and validation flow, clinical analysis configuration/results surfaces, risk-score hub/common actions, and a compact publish/reporting slice including shared document actions, results-table captions/headers, and publication templates for Arabic while preserving canonical identifiers such as `OHDSI`, `Open Targets`, `GWAS`, `Atlas JSON`, `JSON`, `PDF`, `HR`, `IRR`, `AUC`, `AUPRC`, and `I²` where appropriate. The current file-level Arabic distinct counts are `investigationResources` `411/471` and `publishCareGapRiskResources` `348/572`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `50.16%`, clearing the requested 50% milestone on `main`.
- Continued the same Arabic (`ar`) RTL-canary lane on `main` with the next app-shell + study-workbench tranche in `frontend/src/i18n/appResources.ts`, plus cleanup residue passes in `frontend/src/i18n/investigationResources.ts`, `frontend/src/i18n/gisToolsResources.ts`, `frontend/src/i18n/standardProsResources.ts`, `frontend/src/i18n/commonsResources.ts`, and `frontend/src/i18n/smallWorkbenchResources.ts`. This pass added Arabic coverage across the selected study-management, vocabulary, jobs, and data-explorer shells in `appResources`, then closed most of the remaining Arabic tail in the earlier investigation / GIS / Standard PROs / Commons / small-workbench packs while preserving canonical identifiers such as `OHDSI`, `Ares`, `Achilles`, `DQD`, `CDM`, `OMOP`, `JSON`, `CSV`, `LiveKit`, and `MedGemma`. Current file-level Arabic distinct counts now stand at `appResources` `1087/2862`, `investigationResources` `464/471`, `gisToolsResources` `405/414`, `standardProsResources` `424/428`, `commonsResources` `391/395`, and `smallWorkbenchResources` `126/128`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `60.28%`, clearing the requested 60% milestone on `main`.
- Continued the same Arabic (`ar`) RTL-canary lane on `main` with the next two large structured packs in `frontend/src/i18n/analysisResources.ts` and `frontend/src/i18n/cohortDefinitionResources.ts`. This pass added Arabic coverage across the full analysis workspace, including shared analysis chrome, characterization, incidence-rate, pathway, estimation, prediction, SCCS, evidence-synthesis, diagnostics, execution states, and methodological/statistical labels, plus the full cohort-definition authoring workspace covering generation, wizard flow, expression editor, diagnostics, overlap, validation/review, patient/member views, SQL preview, and specialized genomics / imaging / PRS criteria. Canonical identifiers and technical terms such as `OHDSI`, `SCCS`, `AUC`, `AUPRC`, `IRR`, `HR`, `95% CI`, `Kaplan-Meier`, `PheValuator`, `Circe`, `PRS`, `PGS`, `CDM`, `JSON`, and `CSV` were preserved where appropriate, with a manual cleanup pass applied to the obvious Arabic machine-translation traps before landing. Current file-level Arabic distinct counts now stand at `analysisResources` `534/552` and `cohortDefinitionResources` `550/557`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `70.95%`, clearing the requested 70% milestone on `main`.
- Continued the same Arabic (`ar`) RTL-canary lane on `main` with a combined patient-profiles / patient-similarity / imaging / genomics / radiogenomics tranche in `frontend/src/i18n/profileSimilarityResources.ts` and `frontend/src/i18n/imagingGenomicsResources.ts`. This pass added Arabic coverage across patient profiles, cohort browser/search, concept detail, labs/notes/visits/eras/timeline views, the full patient-similarity workspace including diagnostics, matching, embedding/network views, and researcher interpretation, plus the imaging viewer/timeline/criteria workspace, genomics upload/ClinVar/variant-analysis surfaces, and radiogenomics panel chrome. Canonical identifiers and technical terms such as `MRN`, `|SMD|`, `AUC`, `DICOM`, `PACS`, `CT`, `MRI`, `PET`, `ClinVar`, `TMB`, `MSI`, and `HGVS` were preserved where appropriate, followed by a targeted cleanup pass to remove the last visible English holdovers and obvious machine-translation traps before shipping. `profileSimilarityResources.ts` now reports `640/640` distinct Arabic values, `imagingGenomicsResources.ts` now reports `569/569` distinct Arabic values, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `80.79%`, clearing the requested 80% milestone on `main`.
- Continued the same Arabic (`ar`) RTL-canary lane on `main` with a large administration + Ares tranche in `frontend/src/i18n/appResources.ts`. This pass added Arabic coverage across the Administration dashboard, auth providers, roles/users, PACS and Solr administration, AI providers, GIS import, Chroma/vector tooling, Atlas migration, FHIR export and FHIR connections, vocabulary import, system health, FHIR sync, GIS data, Honest Broker, and the `dataExplorer.ares` hub including network overview, DQ history, coverage, feasibility, releases, unmapped-code review, diversity, cost, annotations, and concept-comparison surfaces. Canonical identifiers and technical terms such as `Ares`, `Abby`, `OHDSI`, `OMOP`, `CDM`, `FHIR`, `SMART`, `OAuth 2.0`, `SAML 2.0`, `OpenID Connect`, `LDAP`, `Solr`, `Orthanc`, `DICOMweb`, `PACS`, `GraphQL`, `Dagster`, `GeoJSON`, and `NDJSON` were preserved where appropriate, followed by a targeted manual cleanup pass for token-mangling artifacts and the most visible Arabic admin strings before shipping. `appResources.ts` now reports `2676/2862` distinct Arabic values with app-namespace distinct coverage at `95.59%`, and `npm run i18n:report` continues to show `100%` key presence / `0` missing keys for every locale while improving Arabic overall distinct coverage to `93.89%`, clearing the requested 90% milestone on `main`.
- Completed the Arabic (`ar`) RTL-canary lane to `100% distinct` coverage on `main` with the final residue pass across `frontend/src/i18n/resources.ts`, `frontend/src/i18n/appResources.ts`, `frontend/src/i18n/publishCareGapRiskResources.ts`, `frontend/src/i18n/imagingGenomicsResources.ts`, `frontend/src/i18n/investigationResources.ts`, and `frontend/src/i18n/analysisResources.ts`. This closing pass replaced the remaining English Abby/about shell copy, setup-wizard/auth/setup chrome, study-management and study-designer residue, care-gap and publish workflow tails, Ares/data-explorer status labels, mapping-assistant placeholders, imaging/genomics action headers, the Open Targets investigation tab label, and the last Arabic analysis label tail while preserving reviewed canonical identifiers such as `OHDSI`, `HADES`, `OMOP`, `Arachne`, `FHIR`, `WebAPI`, `Open Targets`, and `AUC` where appropriate. `npm run i18n:report` now reports Arabic at `100% distinct` overall coverage with `100%` key presence and `0` missing keys, making `ar` the completed RTL canary locale alongside the fully completed public locale waves already landed on `main`.
- Closed the final six-locale residue sweep on `main` across `frontend/src/i18n/standardProsResources.ts`, `frontend/src/i18n/imagingGenomicsResources.ts`, and `frontend/src/i18n/profileSimilarityResources.ts`. This cleanup replaced the stray Arabic bleed-through in French/German/Portuguese/Finnish/Japanese/Simplified Chinese Standard PRO labels, filled the last empty imaging/genomics action headers, and restored the final `patientSimilarity.searchForm.mrn` labels for Finnish/Japanese/Simplified Chinese. `npm run i18n:report` now reports every currently supported locale — `en-US`, `es-ES`, `fr-FR`, `de-DE`, `pt-BR`, `fi-FI`, `ja-JP`, `zh-Hans`, `ko-KR`, `hi-IN`, `ar`, and `en-XA` — at `100% distinct` coverage with `100%` key presence and `0` missing keys.
