...
- [x] Triage the full frontend scanner backlog into release-blocking user surfaces, generated/static data surfaces, intentional non-translatable strings, and later extraction waves. Current scan after the data-source/ingestion wave: 5,265 total candidates, 372 FinnGen candidates excluded from this i18n track, and 4,893 non-FinnGen candidates. Detailed triage and extraction order are captured in `docs/superpowers/specs/2026-04-20-i18n-full-scanner-backlog-triage.md`.
- [x] Complete the data-source setup and ingestion extraction wave from the full scanner. `src/features/data-sources`, `src/features/ingestion`, and FHIR ingestion/export chrome now report 0 candidates across 46 files; the full scanner is down to 5,265 total candidates and 4,893 non-FinnGen candidates.
