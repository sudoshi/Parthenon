...
- The full frontend scanner now reports 2,547 total candidates / 2,175 non-FinnGen candidates after the profiles and patient-similarity wave. The current `npm run i18n:scan -- --app-priority` command reports the same 2,547-candidate whole-frontend backlog, so the earlier 0-candidate app-priority shell milestone should be treated as historical rather than the present whole-app extraction gate.
