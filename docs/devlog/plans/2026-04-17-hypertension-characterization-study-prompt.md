# Claude Code Implementation Prompt — Hypertension Characterization Study
...
You are implementing an end-to-end observational hypertension characterization study inside the Parthenon platform. The study was authored by Glenn H. Bock, MD ("DRAFT: Characterization of hypertension in a large study population", 04/17/2026). Read `docs/devlog/modules/STUDIES.md`, `docs/devlog/modules/analyses/`, and `CLAUDE.md` before writing any code. Query the Parthenon Brain (`parthenon-brain` MCP, collections `parthenon_docs` and `parthenon_code`) for prior art on:
...
**Population.** Adults ≥18 at index, any gender, with ≥3 recorded BPs in the 24 months preceding index, whose average SBP > 130 mmHg OR average DBP > 80 mmHg (2017 ACC/AHA Stage 1+). At index they must have NO prior cardiovascular disease (MI, HF, stroke, PAD, revascularization), NO abnormal kidney function (CKD stage ≥3 or eGFR < 60 on ≥2 occasions 90 days apart), NO thyroid disease (hypo- or hyper-), and NO prior antihypertensive exposure.

**Index event.** Earliest qualifying condition_occurrence of Essential Hypertension (SNOMED 59621000 and descendants) OR the earliest measurement date at which the 2-year rolling average crosses the Stage 1 threshold, whichever comes first. Record BOTH and expose the lag.

**Principal goals (verbatim, mapped to analyses):**

1. Retrospective incidence of new-onset HTN based on office-measured BP → Analysis A (Incidence).
2. Frequency of documented home/ABPM device use, compared to office BP → Analysis B (Device Utilization).
3. Time from first documented elevated BP to HTN diagnosis → Analysis C (Diagnostic Latency).
4. Serial BPs aligned to initial and subsequent medication/class changes → Analysis D (Treatment Trajectory).
5. Patients receiving renal denervation (RDN), with BP at 1, 3, 6, 12 months post-procedure → Analysis E (RDN Sub-cohort).

**Secondary characterization:** age/sex/race/region/anthropometrics, baseline labs (CBC, CMP, lipid panel), renal/cardiac/carotid imaging and function, comorbidity onset after index, all-cause mortality with HTN stage at last known BP.

**Stratifiers:** demographics, population density (rural vs urban from the GIS module), HTN stage at index, number of concurrent antihypertensives, presence of treatment-resistant HTN (≥3 agents including a diuretic with SBP ≥130 or DBP ≥80).
...
- Primary criteria: first occurrence of `htn_essential` OR (rolling 24-month mean of `bp_systolic_office` > 130 OR `bp_diastolic_office` > 80 computed over ≥3 measurements), whichever is earliest. Index date = min of the two.
...
- Inclusion criteria (all must hold at index):
  1. Age ≥ 18 at index.
  2. ≥ 3 BP measurements from `bp_systolic_office` ∪ `bp_diastolic_office` in the 24 months preceding index, at least two of which are on distinct calendar days.
  3. Average SBP > 130 mmHg OR average DBP > 80 mmHg across those measurements (use `MeasurementValueAsNumber`).
...
**O1 — Outcome: Incident MACE** (`cvd_mi` ∪ `cvd_stroke` ∪ `cvd_hf` ∪ cardiovascular death) after index.
...
**Analysis C — Diagnostic latency.**
For each T patient compute `days_from_first_elevated_bp_to_first_htn_dx`. Report median/IQR overall, by stage at index, and by rural vs urban (GIS classification). Kaplan-Meier of time-to-diagnosis.

**Analysis D — Treatment trajectory.**
Use the existing `DrugEraBuilder` to construct continuous eras per RxNorm ingredient. For each T patient, produce an ordered timeline of (era_start, era_end, class). Build state-transition diagrams (first-line → second-line → third-line) using the class buckets defined in §2. Co-register BP measurements within ±30d of each era boundary. Output: Sankey diagram data + per-regimen mean BP trajectory (weeks −12 to +52 relative to era start).
...
4. Is secondary HTN (`htn_secondary`) an exclusion or a separate sensitivity cohort?
