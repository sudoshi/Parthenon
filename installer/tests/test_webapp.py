...
def test_web_installer_launch_step_includes_windows_fields():
    ...
    assert "enter either the Parthenon repo path or the WSL repo path before continuing" in app_js
    assert "wsl_distro: payload.wsl_distro" in app_js
    assert "wsl_repo_path: payload.wsl_repo_path" in app_js


def test_onboarding_modal_no_longer_uses_dry_run_or_blocks_on_umls():
    ...
    assert 'id="dry-run-btn"' in index_html
