  ...
  if (impacts.length === 0) {
    return (
      <p className="py-4 text-center text-xs text-text-ghost">
        {t("dataExplorer.ares.feasibility.impact.noData")}
      </p>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-border-subtle bg-surface-raised p-4">
      <h4 className="mb-1 text-sm font-medium text-text-primary">
        {t("dataExplorer.ares.feasibility.impact.title")}
      </h4>
      <p className="mb-3 text-[11px] text-text-ghost">
        {t("dataExplorer.ares.feasibility.impact.description", {
          passed: formatNumber(baselinePassed),
            ...
            <div key={impact.criterion} className="flex items-center gap-3">
              ...
              <div className="relative h-5 flex-1 rounded bg-surface-overlay">
                ...
                <span className="absolute left-2 top-0 flex h-5 items-center text-[10px] font-bold" style={{ color }}>
                  {t("dataExplorer.ares.feasibility.impact.sourcesRecovered", {
                    count: formatNumber(impact.impact),
                  })}
                </span>
              </div>
      ...
      <div className="mt-3 border-t border-border-subtle pt-2">
        <p className="text-[10px] text-text-ghost">
          {t("dataExplorer.ares.feasibility.impact.guidance")}
        </p>
      </div>
    </div>
  );
