...
export default function SwimLaneTimeline({ data }: SwimLaneTimelineProps) {
  if (data.length === 0) {
    return <p className="text-center text-xs text-[#555]">No release data available.</p>;
  }
    ...
    <div className="space-y-3">
      {data.map((lane, laneIdx) => (
        <div key={lane.source_id} className="flex items-center gap-3">
          <div className="w-32 shrink-0 truncate text-xs text-[#888]" title={lane.source_name}>
            {lane.source_name}
          </div>
          <div className="relative h-8 flex-1 rounded bg-[#1a1a22]">
              ...
              return (
                ...
                >
                  ...
                  <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 whitespace-nowrap rounded bg-[#0E0E11] px-2 py-1 text-[10px] text-[#ccc] opacity-0 shadow-lg group-hover:opacity-100 transition-opacity border border-[#252530]">
                    ...
                    <br />
                    <span className="text-[#666]">{release.type}</span>
                  </div>
                </div>
              );
            })}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-1 justify-between text-[9px] text-[#555]">
