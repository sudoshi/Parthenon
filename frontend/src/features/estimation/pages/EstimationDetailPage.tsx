...
import {
  useEstimation,
  useDeleteEstimation,
  useExecuteEstimation,
  useEstimationExecutions,
  useEstimationExecution,
...
export default function EstimationDetailPage() {
  ...
  const executeMutation = useExecuteEstimation();

  const { data: executions } = useEstimationExecutions(estId);
  ...
  const { data: activeExec } = useEstimationExecution(estId, selectedExecId);

  const handleDelete = () => {
    if (!estId) return;
    if (
      window.confirm(
        t("analyses.auto.areYouSureYouWantToDeleteThisEstimationAnalysis_9355bf"),
      )
    ) {
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <p className="text-critical">
            {t("analyses.auto.failedToLoadEstimationAnalysis_de376d")}
          </p>
          <button
            type="button"
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab content */}
      {activeTab === "design" ? (
        <EstimationDesigner estimation={estimation} />
      ) : (
        <div className="space-y-6">
          <EstimationResults execution={activeExec} />
