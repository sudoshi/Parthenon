...
export default function CommunityWorkbenchSdkDemoPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-border-default bg-surface-darkest/70 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <a
              href="/workbench"
              className="inline-flex items-center gap-2 rounded-lg border border-border-default bg-surface-base/60 px-3 py-2 text-sm font-medium text-zinc-200 transition-colors hover:bg-surface-raised"
            >
      ...
      {isLoading ? (
        <section className="rounded-2xl border border-border-default bg-surface-base/50 p-6 text-sm text-zinc-300">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-2xl border border-border-default bg-surface-base/50 p-6">
          ...
          <pre className="overflow-x-auto rounded-xl border border-border-default bg-surface-darkest/80 p-4 text-xs leading-6 text-zinc-300">
      ...
      <section className="rounded-2xl border border-border-default bg-surface-base/50 p-6">
        ...
        <p className="mb-4 text-sm leading-6 text-zinc-400">
          ...
          <span className="mx-1 rounded bg-surface-darkest px-1.5 py-0.5 font-mono text-xs text-zinc-300">
            {generatedSample?.path ?? "community-workbench-sdk/generated-samples/community_variant_browser"}
          </span>
          in the repository.
        </p>
        {generatedSample?.readme_excerpt ? (
          <pre className="mb-4 overflow-x-auto rounded-xl border border-border-default bg-surface-darkest/80 p-4 text-xs leading-6 text-zinc-300">
            {generatedSample.readme_excerpt}
          </pre>
        ) : null}
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {(generatedSample?.files ?? []).map((entry) => (
            <div key={entry} className="rounded-xl border border-border-default bg-surface-darkest/70 px-4 py-3 font-mono text-xs text-zinc-300">
              {entry}
            </div>
          ))}
        </div>
