        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-success px-3 py-2 text-xs font-medium text-surface-base hover:bg-success transition-colors disabled:opacity-50"
            >
              {syncMutation.isPending && !syncingPapu ? (
                <Loader2 size={12} className="animate-spin" />
              ) : (
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-sm bg-surface-raised border border-border-default rounded-lg text-text-primary placeholder-text-ghost focus:outline-none focus:border-success/50 focus:ring-1 focus:ring-success/30"
              />
            </div>
            <input
              type="text"
              ...
              placeholder="Gene"
              className="w-28 px-3 py-2 text-sm bg-surface-raised border border-border-default rounded-lg text-text-primary placeholder-text-ghost focus:outline-none focus:border-success/50 focus:ring-1 focus:ring-success/30"
            />
            <select
              value={sig}
                    ...
                    {results.data.map((v: ClinVarVariant) => (
                      <tr key={v.id} className="hover:bg-surface-overlay transition-colors">
                        <td className="px-4 py-3 font-semibold text-text-primary text-xs">
                          {v.gene_symbol ?? <span className="text-surface-highlight">—</span>}
                        </td>
                        ...
                        <td className="px-4 py-3 text-xs text-text-muted max-w-[200px] truncate" title={v.disease_name ?? undefined}>
                          {v.disease_name ?? <span className="text-surface-highlight">—</span>}
                        </td>
                        ...
                        <td className="px-4 py-3 text-[10px] text-text-ghost space-y-0.5">
                          ...
                          {v.rs_id && <div className="text-surface-highlight">{v.rs_id}</div>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  {uploads.map((upload: GenomicUpload) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-text-ghost text-xs">
                        ...
                        <span className="text-surface-highlight">({formatBytes(upload.file_size_bytes)})</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button
