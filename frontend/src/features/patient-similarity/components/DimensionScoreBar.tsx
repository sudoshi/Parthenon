...
export function DimensionScoreBar({ score, label }: DimensionScoreBarProps) {
  if (score == null || !Number.isFinite(score)) {
    return (
      <div className="flex items-center gap-1.5" title={`${label}: N/A`}>
        <div
          className="h-2 rounded-full bg-[var(--color-surface-overlay)]"
          style={{ width: 50 }}
        />
        <span className="text-[10px] text-[var(--color-text-muted)] tabular-nums">N/A</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="relative h-2 rounded-full bg-[var(--color-surface-overlay)]" style={{ width: 50 }}>
        <div
