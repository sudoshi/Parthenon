...
export default function SccsDetailPage() {
  ...
  const { data: sccs, isLoading } = useSccsAnalysis(analysisId);
  ...
  const { data: execution, isLoading: loadingExecution } = useSccsExecution(
    analysisId,
    selectedExecutionId,
  );
  ...
  const handleExecute = () => {
    if (!analysisId || !selectedSourceId) return;
    executeMutation.mutate(
      { id: analysisId, sourceId: selectedSourceId },
      {
        onSuccess: (exec) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
          ...
          {!isNew && (
            <>
              <select
                ...
                className={cn(
                  "rounded-lg border border-[#232328] bg-[#0E0E11] px-3 py-2 text-sm",
                  "text-[#F0EDE8] focus:border-[#C9A227] focus:outline-none",
                )}
              >
              ...
              <button
                ...
                disabled={!selectedSourceId || executeMutation.isPending}
      ...
      {activeTab === "results" && !isNew && (
        <SccsResults execution={execution ?? null} isLoading={loadingExecution} />
      )}

      {/* Execution History */}
      {!isNew && sccs?.executions && sccs.executions.length > 0 && (
