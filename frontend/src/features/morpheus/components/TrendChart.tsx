  ...
  return (
    <div className="rounded-lg border border-gray-800 bg-[#1A1A2E] p-4">
      ...
      <svg width="100%" viewBox={`0 0 ${svgWidth} ${height}`} className="overflow-visible">
          ...
          return (
            <g key={i}>
              ...
              {(i % Math.max(1, Math.floor(data.length / 8)) === 0) && (
                <text x={x + barWidth / 2} y={chartHeight + 14} textAnchor="middle" className="text-[8px]" fill="#6B7280">
                  {d.label.slice(2)}
                </text>
              )}
            </g>
        ...
        <line x1={marginLeft - 4} y1={chartHeight} x2={svgWidth - marginRight} y2={chartHeight} stroke="#374151" strokeWidth={1} />

        {barLabel && (
          <text x={4} y={chartHeight / 2} textAnchor="middle" transform={`rotate(-90, 8, ${chartHeight / 2})`}
                className="text-[8px]" fill="#6B7280">{barLabel}</text>
        )}
      </svg>
