...
export default function ConceptDetailDrawer({ event, onClose, dataset }: ConceptDetailDrawerProps) {
  ...
  const color = DOMAIN_COLORS[event.domain as keyof typeof DOMAIN_COLORS] ?? 'var(--text-muted)';
        ...
        <div className="border-t border-surface-highlight px-4 py-3 space-y-2">
          {event.concept_id && (
            <Link
              ...
              onClick={onClose}
              className="flex items-center justify-center gap-1.5 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-success transition-colors hover:bg-zinc-800 focus:outline-none focus:ring-1 focus:ring-success/30"
