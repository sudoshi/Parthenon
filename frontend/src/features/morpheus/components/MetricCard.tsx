...
export default function MetricCard({ label, value, color = '#2DD4BF', subtext, onClick }: MetricCardProps) {
  return (
    <div
      className={`rounded-xl border border-border-default bg-surface-darkest/70 p-4 flex flex-col gap-1 transition-colors ${onClick ? 'cursor-pointer hover:bg-[#1A1A1E]' : ''}`}
