...
export function WikiActivityDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      <div
        className="flex h-full w-full max-w-sm flex-col border-l border-border-default bg-surface-raised shadow-2xl"
              ...
              {activity.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <p
                      className={`text-[10px] font-semibold uppercase tracking-wider ${ACTION_COLORS[item.action] ?? "text-success"}`}
                    >
                      {item.action}
                    </p>
