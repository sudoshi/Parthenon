...
export default function PredictionDetailPage() {
  ...
  const predId = id ? Number(id) : null;

  const {
    data: prediction,
    isLoading,
    error,
  } = usePrediction(predId);
  const deleteMutation = useDeletePrediction();
  const executeMutation = useExecutePrediction();

  const { data: executions } = usePredictionExecutions(predId);
  ...
  const { data: activeExec } = usePredictionExecution(predId, selectedExecId);

  const handleDelete = () => {
    if (!predId) return;
    if (
      window.confirm(
        t("analyses.auto.areYouSureYouWantToDeleteThisPredictionModel_aa1b10"),
      )
    ) {
      deleteMutation.mutate(predId, {
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <p className="text-critical">
            {t("analyses.auto.failedToLoadPredictionModel_ae9d9e")}
          </p>
          <button
            type="button"
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab content */}
      {activeTab === "design" ? (
        <PredictionDesigner prediction={prediction} />
      ) : (
        <div className="space-y-6">
          <PredictionResults execution={activeExec} />
