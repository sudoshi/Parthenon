...
export default function MetadataColorPicker({ metadataKeys, value, onChange }: MetadataColorPickerProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-text-ghost">Color by</span>
      <select
        value={value ?? ""}
        onChange={(e) => onChange(e.target.value || null)}
        className="rounded border border-border-default bg-surface-base px-2 py-1 text-xs text-[#E8E4DC] outline-none focus:border-accent/50"
      >
