...
export function AboutProsModal({ onClose, stats }: AboutProsModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-4xl max-h-[calc(100vh-4rem)] overflow-y-auto rounded-2xl border border-[#2A2A2F] bg-[#0E0E11] shadow-2xl">
        {/* Sticky header */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#2A2A2F] bg-[#0E0E11]/95 backdrop-blur-sm px-6 py-4">
          <div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-2 text-[#8A857D] hover:text-[#F0EDE8] hover:bg-[#1A1A1F] transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="space-y-8 p-6">
          ...
          <section>
            ...
            <p className="text-sm text-[#8A857D] leading-relaxed mt-3">
          ...
          {/* ── The Problem ───────────────────────────────────────────── */}
          <section>
            <SectionTitle icon={AlertTriangle} color="#E85A6B" title="The Problem" />
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className="flex items-center justify-center w-7 h-7 rounded-md bg-[#E85A6B]/10">
                        <Icon size={14} className="text-[#E85A6B]" />
                      </div>
                      <div>
                        <span className="text-[10px] font-['IBM_Plex_Mono',monospace] text-[#5A5650]">
                          Pain Point {pp.id}
                        </span>
                        <h3 className="text-xs font-semibold text-[#F0EDE8]">
                          {pp.title}
                        </h3>
                      </div>
                    </div>
          ...
          {/* ── The Solution ──────────────────────────────────────────── */}
          <section>
            <SectionTitle icon={Lightbulb} color="#C9A227" title="The Parthenon Solution" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {PILLARS.map((pillar) => (
                <div
                  key={pillar.id}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-xs font-semibold text-[#F0EDE8] mb-0.5">
                        {pillar.title}
                      </h3>
                      <p
          ...
          <section>
            <SectionTitle icon={Database} color="#A78BFA" title="Technical Architecture" />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                { table: "app.survey_instruments", desc: "Instrument registry: name, version, domain, scoring, LOINC, OMOP concept, license", color: "#2DD4BF" },
                { table: "app.survey_items", desc: "Individual questions: item text, response type, OMOP concept, subscale, reverse-coding", color: "#C9A227" },
                { table: "app.survey_answer_options", desc: "Answer choices: option text, numeric score, OMOP concept, LOINC LA code", color: "#60A5FA" },
                { table: "app.survey_conduct", desc: "Administration metadata: respondent type, mode, completion status, visit linkage, scores", color: "#A78BFA" },
                { table: "app.survey_responses", desc: "Bridge table linking survey_conduct to observation rows with item-level values", color: "#E85A6B" },
              ].map((s) => (
                <div key={s.table} className="rounded-xl border border-[#2A2A2F] bg-[#141418] p-4">
                  <div className="flex items-center gap-2 mb-1.5">
                    <Code2 size={12} style={{ color: s.color }} />
                    <span className="text-[11px] font-semibold font-['IBM_Plex_Mono',monospace] text-[#F0EDE8]">
                      {s.table}
                    </span>
                  </div>
                  <p className="text-[11px] text-[#8A857D] leading-relaxed">{s.desc}</p>
                </div>
              ))}
              <div className="rounded-xl border border-dashed border-[#C9A227]/30 bg-[#C9A227]/5 p-4 flex items-start gap-2">
                <ArrowRight size={14} className="text-[#C9A227] shrink-0 mt-0.5" />
                <div>
                  <h3 className="text-[11px] font-semibold text-[#C9A227] mb-0.5">CDM v6.0 Forward-Compatible</h3>
                  <p className="text-[11px] text-[#8A857D] leading-relaxed">
          ...
          {/* ── Roadmap ───────────────────────────────────────────────── */}
          <section>
            <SectionTitle icon={Wrench} color="#C9A227" title="Implementation Roadmap" />
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div
                      className={cn(
                        ...
                        idx === 0 ? "bg-[#2DD4BF]/15 text-[#2DD4BF]" : "bg-[#1A1A1F] text-[#8A857D]",
                      )}
                    >
                      {phase.phase}
                    </div>
                    <div>
                      <h3 className="text-xs font-semibold text-[#F0EDE8]">{phase.title}</h3>
                      <span className="text-[10px] font-['IBM_Plex_Mono',monospace] text-[#5A5650]">{phase.duration}</span>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-1.5 ml-9">
                    {phase.items.map((item) => (
                      <span key={item} className="inline-block rounded-md bg-[#0E0E11] border border-[#2A2A2F]/50 px-2.5 py-1 text-[10px] text-[#C5C0B8]">
                        {item}
                      </span>
                    ))}
                  </div>
          ...
          {/* ── Key Insights ──────────────────────────────────────────── */}
          <section>
            <SectionTitle icon={Beaker} color="#60A5FA" title="Key Insights" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <InsightCard
                icon={Lock}
                color="#C9A227"
